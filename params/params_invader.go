...
var invaderParams = []ParamMeta{
	{
		...
		Advanced: false, Min: 1, Max: 20, UnlockMin: 1, UnlockMax: float64(math.MaxInt32),
	...
	},
	{
		...
		Advanced: false, Min: 2, Max: 30, UnlockMin: 2, UnlockMax: 999,
	...
	},
	{
		...
		Advanced: false, Min: 3, Max: 20, UnlockMin: 3, UnlockMax: 999,
		Warning: "Lowering below 7–8 s starts filtering valid cross-region connections — those players can legitimately take 8–20 s to establish. Same-region connections typically complete in 2–5 s; setting the timeout near the same-region floor costs you the cross-region pool without a meaningful speed gain.",
		ShortDef: "Per-candidate handshake deadline. After selecting a host from the list, this is how long the client waits for the session to establish before abandoning that entry and moving to the next.\r\nRaise → slow or cross-regional connections get more time to complete; recommended floor is 7–8 s.\r\nLower → dead sessions cleared faster, but values below 7 s start cutting valid cross-region matches before they finish.",
